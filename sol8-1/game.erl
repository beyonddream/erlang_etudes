...
%% The dealer has to keep track of the player' process IDs,
...
dealer(Pids, State, P1Cards, P2Cards, Count, Pile) ->
    ...
    case State of
        ...
        await_battle ->
           {NextCount, Next_P1Cards, Next_P2Cards} = receive
                {accept, Pid, Data} ->
                    {Next_P1Cards_Tmp, Next_P2Cards_Tmp} = case Pid of
                        P1 -> {Data, P2Cards};
                        P2 -> {P1Cards, Data}
                    end,
                    {Count + 1, Next_P1Cards_Tmp, Next_P2Cards_Tmp}
            end,
            NextState = if
                NextCount == 2 -> check_cards;
                NextCount /= 2 -> State
            end,
            dealer(Pids, NextState, Next_P1Cards, Next_P2Cards, NextCount, Pile);
        check_cards ->
...
player(Dealer, Hand) ->
    NewHand = receive
        {Command, Data} ->
            NewHand2 = case Command of
                give_cards ->
                    {ToSend, NewHand1} = give_cards(Hand, Data),
