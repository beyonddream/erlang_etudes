...
-export([init/1,
         ...
         handle_info/2,
         terminate/2,
         code_change/3]). % gen_sever callbacks
...
handle_call({login, UserName, ServerRef}, From, State) ->
    ...
    {Reply, NewState} = case lists:keymember({UserName, ServerRef}, 1, State) of
        true ->
            {State,
             {error, "User " ++ UserName ++ " already in use."}};
        false ->
            {[{{UserName, ServerRef}, FromPid} | State],
             {ok, "Logged in."}}
    end,
...
handle_call(logout, From, State) ->
    {FromPid, _FromTag} = From,
    {NewState, Reply} = case lists:keymember(FromPid, 2, State) of
        true ->
            {lists:keydelete(FromPid, 2, State),
             {ok, logged_out} };
        false ->
            {State,
             {error, not_logged_in} }
