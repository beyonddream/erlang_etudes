...
-export([init/1,
         ...
         terminate/2,
         code_change/3,
         get_chat_node/0]). % gen_server callbacks
...
handle_call({set_profile, Key, Value}, _From, State) ->
    NewProfile = case lists:keymember(Key, 1, State#state.profile) of
        true -> 
            lists:keyreplace(Key, 1, State#state.profile,
                                              {Key, Value});
        false ->
            [{Key, Value} | State#state.profile]
    end,
    {reply, NewProfile,
...
%% ugly hack; it works by sending itself a call to retrive
